...
(function (globalRoot, Chartist) {
  ...
  let window = globalRoot.window;
  let document = globalRoot.document;
  ...
  Chartist.extend = function (target) {
    let i, source, sourceProp, targetProto;
    target = target || {};
  ...
  Chartist.roundWithPrecision = function(value, digits) {
    let precision = Math.pow(10, digits || Chartist.precision);
  ...
  Chartist.getBounds = function (axisLength, highLow, scaleMinSpace, onlyInteger) {
    ...
    let length = Chartist.projectLength(axisLength, bounds.step, bounds);
    ...
    for (i = bounds.min; i <= bounds.max; i = safeIncrement(i, bounds.step)) {
      let value = Chartist.roundWithPrecision(i);
  ...
  Chartist.createChartRect = function (svg, options, fallbackPadding) {
    ...
    let width = svg.width() || Chartist.quantity(options.width).value || 0;
    let height = svg.height() || Chartist.quantity(options.height).value || 0;
    let normalizedPadding = Chartist.normalizePadding(options.chartPadding, fallbackPadding);
  ...
  Chartist.optionsProvider = function (options, responsiveOptions, eventEmitter) {
    let baseOptions = Chartist.extend({}, options),
      currentOptions,
      mediaQueryListeners = [],
      i;
...
(function(globalRoot, Chartist) {
  ...
  function extend(properties, superProtoOverride) {
    let superProto = superProtoOverride || this.prototype || Chartist.Class;
    let proto = Object.create(superProto);

    Chartist.Class.cloneDefinitions(proto, properties);

    let constr = function() {
      let fn = proto.constructor || function () {},
        instance;
  ...
  function cloneDefinitions() {
    let args = listToArray(arguments);
    let target = args[0];
...
(function(globalRoot, Chartist) {
  ...
  function attr(attributes, ns) {
    ...
    Object.keys(attributes).forEach(function(key) {
      ...
      if (key.indexOf(':') !== -1) {
        let namespacedAttribute = key.split(':');
  ...
  function querySelector(selector) {
    let foundNode = this._node.querySelector(selector);
  ...
  function querySelectorAll(selector) {
    let foundNodes = this._node.querySelectorAll(selector);
  ...
  function foreignObject(content, attributes, className, insertFirst) {
    ...
    let fnObj = this.elem('foreignObject', attributes, className, insertFirst);
  ...
  function removeClass(names) {
    let removedClasses = names.trim().split(/\s+/);
...
(function(globalRoot, Chartist) {
  ...
  function parse(path) {
    ...
    let elements = chunks.map(function(chunk) {
  ...
  function stringify() {
    ...
    return this.pathElements.reduce(function(path, pathElement) {
        let params = elementDescriptions[pathElement.command.toLowerCase()].map(function(paramName) {
  ...
  function clone(close) {
    let c = new Chartist.Svg.Path(close || this.close);
    c.pos = this.pos;
  ...
  function join(paths, close, options) {
    let joinedPath = new Chartist.Svg.Path(close, options);
...
(function(globalRoot, Chartist){
  ...
  function createChart(options) {
    ...
    let chartRect = Chartist.createChartRect(this.svg, options, defaultOptions.padding);
    let axisX, axisY;
...
(function(globalRoot, Chartist){
  ...
  function createChart(options) {
    ...
    if(options.stackBars && data.normalized.series.length !== 0) {
      ...
      let serialSums = Chartist.serialMap(data.normalized.series, function serialSums() {
    ...
    data.raw.series.forEach(function(series, seriesIndex) {
      ...
      let biPol = seriesIndex - (data.raw.series.length - 1) / 2;
      ...
      data.normalized.series[seriesIndex].forEach(function(value, valueIndex) {
        ...
        let metaData = Chartist.getMetaData(series, valueIndex);
...
(function(globalRoot, Chartist) {
  ...
  function createChart(options) {
    ...
    data.raw.series.forEach(function(series, index) {
      ...
      let overlappigStartAngle = Math.max(0, startAngle - (index === 0 || hasSingleValInSeries ? 0 : 0.2));
      ...
      let start = Chartist.polarToCartesian(center.x, center.y, radius, overlappigStartAngle),
